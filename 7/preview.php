...
$sql = "SELECT idcontenttemplate AS wid from content_template where idcustomer = " . $idcustomer . " AND name = '" . $template  . "'";
...
if ($_POST['imagenamostrar'] != "") {
    if ($_POST['imagenamostrar'] == $_FILES['IMG']['name'] || $_FILES['IMG']['name'] == "") {
        ...
        $imagen = $imagenleida;
    } else {    //hay una foto y se puede reemplazar
        ...
        // Local
        //$imagen = "http://".$_SERVER["HTTP_HOST"].$plantillascontenido.'templates/'.$nombre_image;
        // Server
                ...
                <section class="oferta__main">
                    <?php if ($has_bg_img) : ?>
                        ...
                        <?php else : ?>
                            <h1 class="oferta__name"><?php echo $data['title']; ?></h1>
                            <div class="oferta__details group">
                                <figure class="oferta__img" style="background-image: url(<?= $imagen; ?>); background-repeat:no-repeat; background-position: center center; background-size: contain;">
                                </figure>
