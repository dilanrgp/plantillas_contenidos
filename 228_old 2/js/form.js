(function($) {
	var templates 		= $('input[name="TEMPLATE"]'),
		...
		bgs_h			= {
							...
							'TEMPLATE-5' : 'img/templates/bg_price.png',
						},
		submitBtn		= $('#SUBMIT'),
		bg;
	...
	function changeForm ( template ) {
		...
		templateConfig.removeClass('offer offer_1 offer_2 message');
		...
		if(!(template === undefined) && templateNumber !== 5) $('#CONFIG').slideDown();
		if(!(template === undefined) && templateNumber === 5) $('#CONFIG_2').slideDown();
	}
	...
	function validateForm ( template ) {
		...
		switch( templateNumber ) {
			...
			// Precios
			case 5:
				validationMsg += addMsg( fields['PRICE_DIESEL'], 'Diesel' );
				validationMsg += addMsg( fields['PRICE_GASOLINA'], 'Gasolina' );
				break;
