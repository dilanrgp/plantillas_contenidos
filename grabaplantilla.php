...
function grabarplantilla($tiempo, $rutahtml, $html, $atributos, $plantilla, $contenido)
{
    // --- 1) normaliza contenido
    $contenedor = str_replace("'", "\"", $html);
    $contenedor = sanitizeOutput($contenedor);

    // --- 2) procesa imágenes referenciadas en el HTML antes de guardar
    $urls = extraerUrlsImagenes($html);
    foreach ($urls as $url) {
        $path = resolverRutaLocal($url, $rutahtml);
        if (!$path) continue;                  // no es local / no resolvió
        if (!is_file($path) || !is_readable($path) || !is_writable($path)) continue;

        procesarImagenSiExcede($path);
    }
    ...
    // opcional: logging
    $archivo = "1plantilla.txt";
    if ($fch = @fopen($archivo, "w")) {
        fwrite($fch, $rutahtml . $str_nombre . PHP_EOL);
        fclose($fch);
    }
